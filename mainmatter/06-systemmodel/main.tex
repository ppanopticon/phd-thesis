...
\begin{proof}[label=proof:limit_non_commutativity]{Non-commutativity of $\selection$ and $\limit$}{}
    Let $\relation$ be a unary relation with $M = |\relation|$ and $\schema(\relation) = \{ \attribute_R \}$. Let further be $t \left[ \attribute_R \right] = \texttt{RANK} (t) \forall t \in \relation$, i.e., the attribute holds the rank according to the current ordering as fixed value (e.g., created when populating the relation). If $\selection$ and $\limit_k$ were to commute, then following identity would hold for any $x < M \in \symnatural_{>0}$:

    \begin{equation*}
        \selection_{\attribute_R > M - x} (\limit_{x} (\relation)) = \limit_{x} (\selection_{\attribute_R > M - x}(\relation))
    \end{equation*}

    However, we can proof by contradiction that this does not hold since:

    \begin{equation*}
        \selection_{\attribute_R < M - x} (\limit_{x} (\relation)) = \{ t_M \} \stackrel{!}{\neq} \limit_{x} (\selection_{\attribute_R < M - x}(\relation)) = \{ t_{M-x}, t_M \}
    \end{equation*}

    That is, whenever $\limit_k$ removes tuples that match the predicate of $\selection$, the order of execution influences the result and the two operators do not commute. An example for $M = 3$ and $x = 2$ is given.

    \begin{center}
        \begin{tabular}{ l || l |}
            $\relation_{\mathtt{result}}$ & $\attributep_{\mathtt{R}}$ \\ 
            \hline
            \hline
            $t_1$ & $1$ \\
            \hline
...
Finally, one can try to match constellations of functions and replace them with an optimised version. A simple example could be the expression $a * b + c$, i.e., $\mathtt{add}(\mathtt{mul}(\mathtt{a},\mathtt{b}), \mathtt{c})$, which can be replaced by a single \emph{fused multiply-add} function $\mathtt{fma}(\mathtt{a},\mathtt{b},\mathtt{c})$ to avoid loss of precission. In a manner of speaking, trading-in a high-dimensional index for the execution of a \acrshort{dfc} also falls into this category with the difference being that the transformation depends on more than just the function.

\subsection{\texorpdfstring{\acrshort{dfc}s}{DFCs} and High-Dimensional Index Structures}

One identity that is particularly relevant for the execution of proximity based queries is the relationship between the execution of a \acrshort{dfc} and the use of a high-dimensional index. As we have discussed in \Cref{section:index_structures}, there exist many different types of indexing techniques that can be used in different ways. We propose to formalise the use of high-dimensional indexes\footnote{By ``index'' we refer to a conceret implementation rather than a family since all techniques introduced in \Cref{section:index_structures} can be employed in different ways.} by definining equivalence classes between  an index and the relational algebra expressions it replaces. Before we can start to formalise the three classes we must provide a formalisation of the index concept itself, which is constructed in \Cref{definition:tuple_identifier}, \Cref{definition:index} and \Cref{definition:hd_index}.

\begin{definition}[label=definition:tuple_identifier]{Tuple Identifier}{}
...
\begin{definition}[label=definition:index]{Index Structure}{}
...
In simple terms, an index $\mathtt{INDEX}^{\relation}_{I}$ is a copy of $\relation$ with respect to the attribute $\attribute_{\mathtt{TID}}$, i.e., every value $\attribute_{\mathtt{TID}}$ in $\relation$ is contained in $\mathtt{INDEX}^{\relation}_{I}$. This basic identity holds for both traditional and high-dimensional index structures. However,  many of the properties that go beyond this definition, e.g., additional attributes contained in $\mathtt{INDEX}^{\relation}_{I}$, depend on the concrete index implementation.

\begin{definition}[label=definition:hd_index]{High-Dimensional Index Structure}{}
    Let $\relation$ be a relation, $\attribute_f \in \schema (\relation)$ and $\symdfc \colon \domain_f \times \domain_f \times \domain_{1} \ldots \times \domain_{n} \to \symreal$ be a \acrshort{dfc}. We call $\mathtt{INDEX}^{\relation}_{\attribute_f, \symdfc}$ a high-dimensional index of relation $\relation$ with respect to $\attribute_f$ and $\symdfc$ if it satisifies \Cref{definition:index}.
\end{definition}

Using this construct, we can now formalise the use of $\mathtt{INDEX}^{\relation}_{\attribute_f, \symdfc}$ in lieu certain relational expressions related to proximity based queries. Depending on what expression is matched, an index replacement falls into one of three classes according to \Cref{definition:dfc_index_class_1}, \Cref{definition:dfc_index_class_2} and \Cref{definition:dfc_index_class_3}.
...
\begin{example}[label=example:index_replacement]{Index Replacement for \acrshort{knn} Search}{}
    ...
    The result of the \acrshort{knn} search $\relation^{\mathcal{A}_d\uparrow}_{\mathtt{result}} = \lambda_k (\order_{\mathcal{A}_d\uparrow} ( \pi_{\mathcal{A}_{t}, \symdist(\mathcal{A}_{f})} ( \relation_p)))$  can be produced by a class 3 index replacement with $\mathtt{INDEX}^{\relation_{p}}_{\attribute_f, \symdfc}$. Therefore:

    \begin{equation*}
        \relation^{\mathcal{A}_d\uparrow}_{\mathtt{result}} = \lambda_k (\order_{\mathcal{A}_d\uparrow} ( \pi_{\mathcal{A}_{t}, \symdist(\mathcal{A}_{f})} ( \relation_p))) = \mathtt{INDEX}^{\relation_{\mathtt{p}}}_{\attribute_f, \symdfc} \Join_{\attribute_{\mathtt{TID}}} \projection_{\attribute_{t}} (\relation_p)
    \end{equation*}
    
    A potential index structure could be the \acrshort{vaf} index \cite[]{Weber:1998Va}.
\end{example}

In addition to \Cref{definition:dfc_index_class_1}, \Cref{definition:dfc_index_class_2} and \Cref{definition:dfc_index_class_3}, the following restrictions may apply in addition to implementation specific restrictions for a specific index:
\begin{enumerate*}[label=(\roman*)]
    ...
    \item High-dimensional indexes are sometimes trained for a specific distance function (e.g., Euclidean) or class of distance functions (e.g., Minkowski). Therefore, an index can only replace a DFC if that DFC coincides with the original function (-class).
\end{enumerate*}

With the aforementioned definitions and restrictions, we have defined the basic relationship and rules that can be applied by a multimedia \acrshort{dbms}, when deciding whether or not to use a particular high-dimensional index structure. However, the definitions so far consider the replacement an exact operation, i.e., the result of the original relational expression and the expression that uses the index is supposed to be identical. In practice, this is often not the case and we will propose an extension to accomodate inexact index structures in \Cref{section:cost_model}.  
...
\begin{example}[label=example:cost_model]{Cost of a Query Execution Plan}{}
    Again using the relations $\relation_{\mathtt{painting}}$ and $\relation_{\mathtt{artist}}$ from \Cref{example:rel_alg_query} and \Cref{example:sql_query}, the query ``return the names of all paintings that were painted by an artist who died after 1800'' results in the following, unoptimised execution plan.
...
Since the atomic costs of any cost stem from completely different domains and are therefore not directly comparable (e.g., memory costs may be measured im bytes wheras IO costs may be measured in number of page accesses), we require a normalisation step for relative comparison of execution plans. That normalisation step is outline in \Cref{definition:cost_normalisation}.
...
\begin{definition}[label=definition:cost_normalisation]{Cost Normalisation}{}
    ...
    It follows from the definition, that each component of a normalised cost falls in an interval between $0$ and $1$ due to the normalisation, i.e., $c_{\mathtt{*},\mathtt{norm}} \in [ 0, 1]$.
\end{definition}

It is important to not that the normalised cost can only be used for relative comparison based on an existing set of plans. It generally does not provide any information about the original cost value. Using this normalised costs, we can now derive \emph{cost score} according to \Cref{definition:cost_score}.

\begin{definition}[label=definition:cost_score]{Basic Cost Score}{}
...
The definitions employed so far treat every component of a cost $C_{\mathcal{P}_{\mathtt{norm}}}$ equally with regards to the resulting cost score $\mathcal{S}$. This is rarely desirable since the different component influence the outcome (e.g., in terms of query execution time) to a different extent. This can be expressed by a \emph{cost policy}  
...
\begin{definition}[label=definition:cost_policy]{Cost Policy and Weighted Score}{}
    ...
    a cost-policy, with $w_{\mathtt{IO}}, w_{\mathtt{CPU}}, w_{\mathtt{MEM}}, w_{\mathtt{ACC}} \ in [0, 1]$. 
    
    The cost score $\mathcal{S}_{\mathcal{W}}$ with respect to $\mathcal{W}$ can be expressed as the linear combination of $C_{\mathcal{P}_{\mathtt{norm}}}$ and $\mathcal{W}$, i.e., 
