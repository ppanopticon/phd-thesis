...
\begin{description}
    ...
    \item[Cost Model for Accuracy] is a direct consequence of \Cref{requirement:big_data} because, as we will demonstrate, responding to changes in the data can lead to the deterioration of indexing quality. Hence, the multimedia database is forced to decide between one over the other. Since some use cases may tolerate such deterioration while other's do not, the contribution also directly addresses \Cref{requirement:accuracy_model}.
\end{description}