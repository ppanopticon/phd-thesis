...
Since the contributions of this Thesis rely on decades of database research, we use this chapter to provide an overview over the relevant fundamentals, to survey related work and to establish the necessary terminology. Most of the fundamentals are guided by \cite{Garcia:2009Database} and \cite{Petrov:2019Database}, which provide an excellent introduction into the field of database systems research.
...
The result is a relation that contains all the attributes of $\relation_L$ and $\relation_R$, i.e., $\schema(\relation_L \Join \relation_R) = \schema(\relation_L) \cup \schema(\relation_R)$, however, the shared attributes are only retained once.
