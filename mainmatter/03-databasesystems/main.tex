...
In order to formalise the structure of and the operations that can be executed on the data represented by a relation, one can turn to \Cref{definition:relation}. 
...
The notion of a relation provides us with the basic tools for data modelling. In addition to \Cref{definition:relation}, Codd proposed a range of constraints to guarantee proper data definition using the following constructs~\cite{Codd:1970Relational,Codd:1972Further}:

\begin{description}
    \item[A \acrfull{ck}] $\mathcal{K}$ of relation $\relation$ is a minimal (or irreducible) set of attributes $\mathcal{K} \subseteq \schema(\relation)$ that uniquely identify any tuple $t \in \relation$. That is, the attributes that are not part of a \acrshort{ck} and the associated values are determined by the choice $\mathcal{K}$ and it is thus impossible to find two distinct tuples that agree on all attributes in $\mathcal{K}$. \acrshort{ck}s can be found by determining the functional dependencies in a relation $\relation$ and applying Armstrong's axioms \cite{Armstrong:1974Dependency}.
    
    Using $\relation_{\mathtt{painting}}$ from \Cref{example:relational_table}, a painting's artist and the year of its conception are functionally determined by the the title of the painting and therefore $\attribute_\mathtt{title} \rightarrow \attribute_{\mathtt{artist}},\attribute_{\mathtt{year}}$. Furthermore, the date of birth and death are implied by the artist, and thus, $\attribute_\mathtt{artist} \rightarrow \attribute_{\mathtt{birth}},\attribute_{\mathtt{death}}$. Given these two dependencies, there is a \emph{superkey} $(\attribute_\mathtt{title}, \attribute_{\mathtt{artist}})$ that determines all the other attributes. However, a \acrshort{ck} must also be minimal and $\attribute_{\mathtt{artist}}$ is implied by $\attribute_\mathtt{title}$ (transitivity). Hence, the \acrshort{ck} for $\relation_{\mathtt{painting}}$ is $\attribute_\mathtt{title}$.
...
\begin{description}
    ...
    Given $\relation_{\mathtt{paintings}}$ in \Cref{example:relational_table}, we must acknowledge that it is not in \acrshort{3nf} since, as we have argued, $\attribute_\mathtt{title} \rightarrow \attribute_{\mathtt{birth}},\attribute_{\mathtt{death}}$ is a transitive dependency, since, $\attribute_\mathtt{title} \rightarrow \attribute_{\mathtt{artist}}$ and $\attribute_{\mathtt{artist}} \rightarrow \attribute_{\mathtt{birth}},\attribute_{\mathtt{death}}$. To resolve this, we can split the relation into two relations as done in \Cref{example:relational_table_pkfk}. The attribute $\attributef_{\mathtt{artist}} \in \relation_{\mathtt{painting}}$ now becomes a \acrshort{fk}.
